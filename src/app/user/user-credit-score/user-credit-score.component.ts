...
export class UserCreditScoreComponent implements OnInit {
  ...
  delete(id: any) {
    this.getCreditScoreservice.deleteCreditScore(id, this.subAdminId).subscribe((response: any) => {
      if (response.success == "1") {
        this.getList();
      } else {
  ...
  updateCreditScore() {
    this.getCreditScoreservice.updateCreditScore(this.creditScoreId, this.myForm.controls.score1.value,
      this.myForm.controls.score2.value, this.myForm.controls.credit_score_name.value,
      this.myForm.controls.credit_score_description.value, this.subAdminId).subscribe((response: any) => {
  ...
  submit() {
    this.getCreditScoreservice.addCreditScore(this.myForm.controls.score1.value, this.myForm.controls.score2.value, this.myForm.controls.credit_score_name.value,
      this.myForm.controls.credit_score_description.value, this.subAdminId).subscribe((response: any) => {
